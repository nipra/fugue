 ...
 *
 * Note: htmlPage is NOT declared volatile in Weblink as unlock on a monitor 'happens before' every
 * subsequent lock on that same monitor.
 *
 * Limitation: Solves task cooperation in a low-level fashion. Josh Bloch says it is like
 * programming in "concurrency assembly language" Synchronized blocks are needed
 *
    ...
    public void run() {
      try {
        synchronized (weblink) {
          ...
          weblink.notifyAll(); // notify() wakes up single thread (chosen arbitrarily if multiple
      ...
      synchronized (weblink) {
        ...
        while (htmlPage == null) {
          try {
            ...
            weblink.wait(); // Suspends thread. Releases lock (unlike sleep()/yield()). Goes into
                            // WAITING state
